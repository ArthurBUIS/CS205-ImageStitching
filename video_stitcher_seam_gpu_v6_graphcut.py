"""
Real-time video stitching from two fixed cameras — v6: graph-cut seam.

Same as v5, with one major change: the seam between the two warped views
can now be computed by graph cut (Boykov-Kolmogorov max-flow / min-cut)
in addition to the original DP approach.

Why graph cut
-------------
DP produces a 1D seam — a function seam_x(y) with one column per row,
with the constraint that consecutive rows differ by at most 1 column.
That works well when the seam is monotonically vertical, but it cannot
route around an obstacle. Concretely: if a chair sits in the middle of
the overlap region, DP must enter the obstacle from above and exit from
below (or be diverted entirely to one side). It cannot go up over the
obstacle, around it, and back down.

Graph cut produces a 2D binary partition of the overlap region: each
pixel is assigned to either "take from A" or "take from B." The
boundary between the two regions IS the seam, and it can have any
shape — including looping around obstacles. With a strong penalty on
foreground/person pixels, graph cut naturally routes the seam around
them without any need for explicit per-blob L/R decisions.

Tradeoffs
---------
  - Cost: graph cut is significantly slower than DP. On a 4x-downscaled
    cost map, expect ~10-50 ms per frame depending on graph density.
    DP runs in ~5 ms.
  - Temporal stability: DP's narrow constraint (±1 column per row)
    naturally smooths the seam frame-to-frame. A 2D partition is more
    flexible and can flicker. We add an optional temporal regularizer
    that biases each pixel toward its previous frame's assignment;
    tune via --gc_temporal_weight.
  - Dependency: PyMaxflow (`pip install PyMaxflow`).

Selection
---------
  --seam_method dp        Default; original DP behavior.
  --seam_method graphcut  Use graph cut instead.

The graph cut path drops --seam_lambda (the DP regularizer) and
--seam_edge_margin (DP edge penalty) — these are DP-specific. Graph
cut uses --gc_temporal_weight for stability and just relies on the
existing FG/person penalties for obstacle avoidance.
...
def compute_fg_mask_seg_gpu(segmenter, frame_a, frame_b, class_ids,
    ...
    # Restrict to true overlap shape (no penalty outside it).
...
def find_graphcut_seam(cost, prev_partition=None, temporal_weight=0.0):
    """
    Compute a binary partition of `cost` (shape H x W) into "take from A"
    (label 0) and "take from B" (label 1) using min-cut / max-flow.

    Cost formulation
    ----------------
    The DP cost map represents "how bad is it to cut through this pixel."
    We map this to a graph cut formulation as follows:

      * Smoothness edges (the "seam cost"): for each pair of 4-connected
        neighbors (p, q), edge weight = (cost[p] + cost[q]) / 2. This is
        what gets paid when p and q end up on opposite sides of the cut.

      * Terminal edges (the "data cost"): we anchor the LEFT edge of the
        bbox to label 0 (A) and the RIGHT edge to label 1 (B), with
        infinity weights. This forces the partition to span left-to-right
        the way DP did. Without this anchoring, the trivial all-A or all-B
        partition would have cost 0 and graph cut would return it.

      * If prev_partition is provided and temporal_weight > 0, add a small
        bias to terminal edges of every pixel: a discount equal to
        temporal_weight * median_cost for the label that pixel had in the
        previous frame. This nudges the partition toward stability without
        preventing real changes.

    Args:
        cost: (H, W) float32 numpy array. Higher = more expensive to cut.
        prev_partition: (H, W) uint8 or None. 0 or 1 per pixel from last
                        frame's partition. Used only if temporal_weight > 0.
        temporal_weight: float, typically 0.0-0.5. 0 disables.

    Returns:
        partition: (H, W) uint8 numpy array. 0 = take from A, 1 = take from B.
    """
    try:
        import maxflow
    except ImportError as e:
        raise RuntimeError(
            "graph cut requires PyMaxflow: pip install PyMaxflow"
        ) from e

    H, W = cost.shape

    # Normalize cost into something safe for int conversion. PyMaxflow
    # accepts float weights but internally truncates; very large values
    # (we use 1e9 for forbidden regions) cause precision issues. We cap
    # at a large but finite value.
    safe_cost = np.minimum(cost, 1e8).astype(np.float32)

    # Smoothness weights from cost: average of neighbor costs.
    # Vertical edges: between (y, x) and (y+1, x).
    vert_weights = (safe_cost[:-1, :] + safe_cost[1:, :]) * 0.5
    # Horizontal edges: between (y, x) and (y, x+1).
    horiz_weights = (safe_cost[:, :-1] + safe_cost[:, 1:]) * 0.5

    # Build the graph.
    g = maxflow.Graph[float]()
    nodeids = g.add_grid_nodes((H, W))

    # Add smoothness edges (4-connectivity).
    # add_grid_edges is the convenient path: it adds undirected edges
    # with the same weight in both directions.
    # We use a custom structure to apply our per-edge weights.
    # add_grid_edges(nodeids, weights=array, structure=...) is for
    # structure-tensor-style; for variable per-edge weights we add
    # them ourselves.
    # For performance, do this with the API that takes per-edge arrays:
    # we pad to make weights match nodeids dims and use 'symmetric=True'.
    #
    # Simpler approach: vertical and horizontal edges added separately
    # via add_grid_edges with shaped structures.

    # Vertical edges: each (y, x) to (y+1, x).
    structure_vert = np.array([[0, 0, 0],
                                [0, 0, 0],
                                [0, 1, 0]])
    # We pad vert_weights from (H-1, W) to (H, W) — the last row's
    # weight is unused because there's no row below it.
    vw_padded = np.zeros((H, W), dtype=np.float32)
    vw_padded[:-1, :] = vert_weights
    g.add_grid_edges(nodeids, weights=vw_padded,
                     structure=structure_vert, symmetric=True)

    # Horizontal edges: each (y, x) to (y, x+1).
    structure_horiz = np.array([[0, 0, 0],
                                 [0, 0, 1],
                                 [0, 0, 0]])
    hw_padded = np.zeros((H, W), dtype=np.float32)
    hw_padded[:, :-1] = horiz_weights
    g.add_grid_edges(nodeids, weights=hw_padded,
                     structure=structure_horiz, symmetric=True)

    # Anchor terminals: left column to source (label 0 = A), right column
    # to sink (label 1 = B). Use a very large weight so these pixels are
    # forced to their assigned terminal.
    INF = 1e10
    sourcecaps = np.zeros((H, W), dtype=np.float32)
    sinkcaps = np.zeros((H, W), dtype=np.float32)
    sourcecaps[:, 0] = INF       # leftmost column: must be source (A)
    sinkcaps[:, -1] = INF        # rightmost column: must be sink (B)

    # Temporal regularizer: a small bias toward each pixel's previous
    # assignment. Implementation: pixels that were A last frame get a
    # small extra edge to source; pixels that were B last frame get a
    # small extra edge to sink.
    if prev_partition is not None and temporal_weight > 0.0:
        # Scale relative to typical cost so the bias is meaningful but
        # not overwhelming. Use the median of the safe cost as reference.
        ref_cost = float(np.median(safe_cost))
        bias = temporal_weight * ref_cost
        was_a = (prev_partition == 0)
        was_b = (prev_partition == 1)
        # was_a: keep on A side -> increase source edge
        # was_b: keep on B side -> increase sink edge
        sourcecaps = sourcecaps + bias * was_a.astype(np.float32)
        sinkcaps = sinkcaps + bias * was_b.astype(np.float32)
    ...
    # Read partition. get_grid_segments returns True where the node is
    # connected to the SINK (i.e. label 1 = B).
    partition_bool = g.get_grid_segments(nodeids)
    partition = partition_bool.astype(np.uint8)
    return partition


def upscale_partition(partition_small, bbox_shape, downscale):
    """
    Upsample a downscaled binary partition to the bbox resolution using
    nearest-neighbor (we want crisp boundaries to drive the soft-mask
    blur).
    """
    H_bb, W_bb = bbox_shape
    if partition_small.shape == (H_bb, W_bb):
        return partition_small
    p_full = cv2.resize(partition_small, (W_bb, H_bb),
                        interpolation=cv2.INTER_NEAREST)
    return p_full


# ---------------------------------------------------------------------------
# Soft mask construction — supports both 1D seam and 2D partition
# ---------------------------------------------------------------------------

def build_soft_mask_fast(seam_x_full, bbox_shape, static, blend_width):
    """
    Original 1D-seam version. Returns a soft mask: ~1 where seam_x is
    > col_idx (left of seam = A side), ~0 elsewhere, blurred.
    """
    H_bb, W_bb = bbox_shape
    ...
    return _blur_and_clamp_mask(hard, bbox_shape, static, blend_width)


def build_soft_mask_from_partition(partition_full, bbox_shape, static, blend_width):
    """
    2D-partition version. Takes a binary partition (0=A, 1=B) and produces
    the same soft mask convention as build_soft_mask_fast: ~1 on the A
    side, ~0 on the B side, blurred.
    """
    # Convert partition: 1.0 where A (partition == 0), 0.0 where B.
    hard = (partition_full == 0).astype(np.float32)
    return _blur_and_clamp_mask(hard, bbox_shape, static, blend_width)


def _blur_and_clamp_mask(hard, bbox_shape, static, blend_width):
    """Common pyramid-blur + overlap-clamp logic."""
    H_bb, W_bb = bbox_shape
...
# ---------------------------------------------------------------------------
# Composite — multi-band blend (GPU)
# ---------------------------------------------------------------------------
...
def composite_multiband_gpu_resident(warped_a_t, warped_b_t, static,
                                     soft_mask_np,
                                     blend_width, blend_levels, out_buf,
                                     gpu_ctx):
    """
    Same as before but takes a precomputed soft mask (numpy) instead of
    a 1D seam_x_full. The caller is responsible for building the mask
    (so DP and graph cut can both feed in their respective masks).
    """
    ...
    H_bb = y1 - y0
    W_bb = x1 - x0
    mask_t = torch.from_numpy(soft_mask_np).to(device, non_blocking=True)
...
def composite_multiband_cpu(warped_a, warped_b, static, soft_mask_np,
                            blend_width, blend_levels, out_buf):
    """
    CPU variant. Takes precomputed soft mask, like the GPU version.
    """
    x0, y0, x1, y1 = static["overlap_bbox"]
    out_buf.fill(0)
    ...
    gp_m = build_gaussian_pyramid_cpu(soft_mask_np, levels)
...
# ---------------------------------------------------------------------------
# Feathered fallback (1D-seam only — used only with --no_multiband + DP)
# ---------------------------------------------------------------------------
...
class PersonSegmenter:
    ...
    def predict_classes_mask_gpu(self, frame_bgr, target_hw, class_ids):
        ...
        Returns the union of all detected instance masks across `class_ids`.
        """
        H_tgt, W_tgt = target_hw
        results = self.model.predict(
...
def draw_seam_overlay_dp(canvas, seam_x_full, bbox):
    """Original DP variant: draw a 1D polyline."""
    x0, y0, x1, y1 = bbox
    H_bb = y1 - y0
...
def draw_seam_overlay_partition(canvas, partition_full, bbox,
                                color=(0, 0, 255), thickness=2):
    """
    Graph-cut variant: draw the boundary between A-pixels and B-pixels.
    """
    x0, y0, x1, y1 = bbox
    # Find boundary via morphological gradient (dilation - erosion).
    p_u8 = (partition_full * 255).astype(np.uint8)
    # 3x3 structural element gives a ~1px boundary; thicken via dilation.
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    grad = cv2.morphologyEx(p_u8, cv2.MORPH_GRADIENT, kernel)
    if thickness > 1:
        kt = cv2.getStructuringElement(cv2.MORPH_RECT,
                                        (thickness, thickness))
        grad = cv2.dilate(grad, kt)
    # Overlay: paint the gradient pixels in `color` on the canvas.
...
def main():
    ...
    parser.add_argument("--seam_lambda", type=float, default=5.0,
                        help="DP-only: temporal regularizer strength.")
    parser.add_argument("--seam_edge_margin", type=int, default=50,
                        help="DP-only: forbidden-band width at bbox left/right.")
    # *** Graph cut flags ***
    parser.add_argument("--seam_method", choices=["dp", "graphcut"],
                        default="dp",
                        help="Seam-finding algorithm. dp (default): "
                             "original 1D dynamic programming. graphcut: "
                             "2D min-cut via PyMaxflow.")
    parser.add_argument("--gc_temporal_weight", type=float, default=0.1,
                        help="Graph-cut-only: temporal stability bias "
                             "(0 = off; ~0.05-0.3 typical). Each pixel "
                             "gets a small bias toward its previous "
                             "frame's assignment, scaled by the median "
                             "data cost.")
    ...
    print(f"[seam] method = {args.seam_method}")
    if args.seam_method == "graphcut":
        print(f"[seam] gc_temporal_weight = {args.gc_temporal_weight}")
        # Verify PyMaxflow is importable.
        try:
            import maxflow  # noqa: F401
        except ImportError:
            raise RuntimeError(
                "--seam_method graphcut requires PyMaxflow. "
    ...
    # --- Static foreground mask (segmentation-based, GPU + CPU) ----------
    # Must run AFTER the GPU init block above because it needs grid_a_t /
    # grid_b_t / overlap_in_bbox_t to exist. On CPU those are built below.
    use_fg = not args.no_fg
    fg_recompute_frames = (
        int(round(args.fg_recompute_seconds * sync_reader.output_fps))
        if args.fg_recompute_seconds > 0 else 0
    )
    # CPU equivalents of the GPU-built tensors. These are needed because
    # compute_fg_mask_seg_gpu uses grid_sample / max_pool2d which work on
    # CPU torch tensors too — but the tensors themselves were only built
    # in the GPU branch. Build them here on the CPU device when missing.
    if use_fg and not dev["cuda_available"]:
        cpu_device = torch.device("cpu")
        grid_a_t = build_grid_sample_tensor(map_ax, map_ay, frame_a.shape, cpu_device)
        grid_b_t = build_grid_sample_tensor(map_bx, map_by, frame_b.shape, cpu_device)
        overlap_in_bbox_t = torch.from_numpy(static["overlap_in_bbox"]).to(cpu_device)
    if use_fg:
    ...
    # State for graph-cut path.
    partition_prev_small = None

    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    raw_writer = cv2.VideoWriter(args.output, fourcc, sync_reader.output_fps,
                                 canvas_size)
    ...
    try:
        while True:
            ...
            if dev["cuda_available"]:
                ...
                # Note: this v6_graphcut starts from v5; FG mask system
                # was added in later versions and is not present here.
                ...
                # FG penalty (CPU). Apply BEFORE person to mirror the GPU
                # ordering — person dominates either way (1e8 > 5e7).
                if use_fg and fg_mask_bbox_t is not None:
                    fg_cpu = fg_mask_bbox_t.cpu().numpy()
                    if person_mask_bbox.any():
                        fg_only = cv2.bitwise_and(
                            fg_cpu, cv2.bitwise_not(person_mask_bbox)
                        )
                    else:
                        fg_only = fg_cpu
                    cost_for_dp[fg_only > 0] += args.fg_penalty
                if person_mask_bbox.any():
                    cost_for_dp[person_mask_bbox > 0] += PERSON_PENALTY

            # Edge margin only meaningful for DP (graph cut anchors L/R
            # terminals at bbox edges already, which is even stronger).
            if args.seam_method == "dp":
            ...
            if args.seam_method == "dp":
                ...
                # Build soft mask from the 1D seam.
                soft_mask_np = build_soft_mask_fast(seam_x_full, bbox_shape,
                                                    static, args.blend_width)
                partition_full = None  # not used in DP path
            else:
                # Graph cut.
                partition_small = find_graphcut_seam(
                    cost_small,
                    prev_partition=partition_prev_small,
                    temporal_weight=args.gc_temporal_weight,
                )
                partition_prev_small = partition_small.copy()
                partition_full = upscale_partition(partition_small,
                                                   bbox_shape, ds)
                soft_mask_np = build_soft_mask_from_partition(
                    partition_full, bbox_shape, static, args.blend_width
                )
                seam_x_full = None  # not used in graphcut path
            ...
            if use_multiband:
                if gpu_ctx is not None:
                    stitched = composite_multiband_gpu_resident(
                        warped_a_t, warped_b_t, static, soft_mask_np,
                        args.blend_width, args.blend_levels, out_buf,
                        gpu_ctx,
                    )
                else:
                    stitched = composite_multiband_cpu(
                        warped_a, warped_b, static, soft_mask_np,
                        args.blend_width, args.blend_levels, out_buf,
                    )
            else:
                # Feathered fallback only works for DP (1D seam).
                if args.seam_method != "dp":
                    raise RuntimeError(
                        "--no_multiband is only supported with --seam_method dp."
                    )
                if warped_a is None:
            ...
            if args.debug_mask:
                if use_fg and fg_mask_bbox_t is not None:
                    fg_cpu = fg_mask_bbox_t.cpu().numpy()
                    draw_mask_overlay(stitched, fg_cpu,
                                      static["overlap_bbox"],
            ...
            if args.debug_seam:
                if args.seam_method == "dp":
                    draw_seam_overlay_dp(stitched, seam_x_full,
                                         static["overlap_bbox"])
                else:
                    draw_seam_overlay_partition(stitched, partition_full,
                                                static["overlap_bbox"])
